...
\newpage
\chapter*{Pourquoi nous avons créé cette application?}

Il y a quelques raisons pour lesquelles cette application est nécessaire:
\begin{itemize}
\item les gens sont occupés et ne peuvent pas toujours préparer leur propre nourriture
\item le client a besoin de demander au restaurant son menu par téléphone
\item le processus de commande de nourriture se fait par téléphone, ce qui peut entraîner des erreurs dans la commande et ne permet pas trop de flexibilité
\item le processus de livraison existant est inefficace, il nécessite plusieurs appels téléphoniques pour identifier l'emplacement ou l'adresse du client
\item il n'existe pas de moyen de communication clair qui facilite la commande et la livraison de nourriture.
\end{itemize}

Et bien d'autres problèmes à régler. Nous avons donc pensé à une solution pour chaque problème et l'avons incluse dans notre application.

\addcontentsline{toc}{chapter}{Pourquoi nous avons créé cette application?}
